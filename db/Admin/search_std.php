...
<div class="search_result">		
    <table class="tab_one">
        <?php 
            $min_length = 1;
            if(isset($_GET['src_student'])) {
                $key = $_GET['src_student'];
                if(strlen($key) >= $min_length){
                    $key = htmlspecialchars($key);
                    $sql = "SELECT * FROM student WHERE name LIKE '%$key%' OR ID LIKE '%$key%'";
        ...
        <tr>
            ...
            <td><a href="admin_single_student.php?id=<?php echo urlencode($rows['ID']);?>">View Details</a></td>
            ...
            <td>
                <?php if(!empty($rows['pic'])): ?>
                    <img src="Simage/<?php echo htmlspecialchars($rows['pic']);?>" title="<?php echo htmlspecialchars($rows['name']);?>" />
                <?php else: ?>
                    <span>No Photo</span>
                <?php endif; ?>
            </td>
        ...
        ?>
        <div class="back fix">
            <p style="text-align:center"><a href="view_std.php"><button class="editbtn">Back to student list</button></a></p>
        </div>
</div>
</body>
</html>