...
$sql="select name,dept,designation,pic, dob from teacher where tid='$sid'";
...
$info_sql = "SELECT * FROM fac_info WHERE tid='$sid'";
...
if(isset($_POST['update_profile'])) {
    ...
    if(isset($_FILES['photo']) && $_FILES['photo']['error'] == 0) {
        ...
        if(move_uploaded_file($_FILES['photo']['tmp_name'], $upload_path)) {
            ...
            // Update photo in teacher table
            $photo_sql = "UPDATE teacher SET pic='$profile_image' WHERE tid='$sid'";
            mysqli_query($con, $photo_sql);
        }
    }

    // Update mobile number in teacher table (if changed)
    // $new_mobile = mysqli_real_escape_string($con, $_POST['mobileno']);
    // if($new_mobile != $p) {
    //     $mobile_sql = "UPDATE teacher SET mobileno='$new_mobile' WHERE tid='$sid'";
    ...
    // Check if record exists in fac_info table
    $check_sql = "SELECT tid FROM fac_info WHERE tid='$sid'";
    $check_result = mysqli_query($con, $check_sql);
    
    if(mysqli_num_rows($check_result) > 0) {
        // Update existing record
        $update_sql = "UPDATE fac_info SET 
                       birth='$birth', 
                       gender='$gender', 
                       blood='$blood', 
                       nation='$nation', 
        ...
        // Insert new record
        $insert_sql = "INSERT INTO fac_info (
                       tid, birth, gender, blood, nation, religion, 
                       email, address
                       ) VALUES (
                       '$sid', '$birth', '$gender', '$blood', '$nation', '$religion', 
                       '$email', '$address'
                       )";
...
echo "<div style='text-align: middle; padding: 20px 0;'>
        <img src='../Admin/Timage/$image' style='width: 220px; height: 220px; border-radius: 50%; object-fit: cover; border: 3px solid #ddd;'>
      </div>";    
?>
</div>
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-grid">
                            <div class="form-group">
                                ...
                                <input type="date" id="birth" name="birth" value="<?php echo htmlspecialchars($info['birth'] ?? ''); ?>">
                            </div>
                            <div class="form-group">
